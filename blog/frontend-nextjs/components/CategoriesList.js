...
//   const { token } = useAuth(); // Get JWT token from context or however you store it
...
//       });
//       if (!res.ok) {
//         throw new Error("Failed to load categories: " + (await res.text()));
//       }
...
export default function CategoriesList() {
  ...
  const loadCategories = async () => {
    try {
      ...
      // Use apiService which automatically handles the token
